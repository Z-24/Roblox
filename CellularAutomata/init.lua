...
local function GetClosestCells(Board: board, Host: cell, PositionY: number, PositionX: number)
    local Moves = {}
    local MovableCells = {}
    --[[
    ...
    for _, Move in pairs(Moves) do
        if Board[Move[1]] ~= nil then
            if Board[Move[1]][Move[2]] ~= nil then
                ...
                if PosCell.Name ~= Host.CellName then
                    --[[ 
...
local function InfectClosestCells(Board: board, Host: cell)
    ...
    local MovableCells = GetClosestCells(Board, Host, Host.Position[1], Host.Position[2])
...
function CellularAutomata:StartSimulation()
    local RandomRow = Random.new()
    local RandomColumn = Random.new()
    local Rng = Random.new()
    for CellName, Stats in pairs(Config.Cells) do
        if Stats.Enabled then
            self.Board[Rng:NextInteger(1, MapSizeX)][Rng:NextInteger(1, MapSizeY)]:Spawn(CellName)
        end
    end 
    --[[
    ...
    coroutine.wrap(function()
        while not Pandemic do
            coroutine.wrap(function()
                for i = 0, 10 do
                    local NextCell = self.Board[RandomRow:NextInteger(1, MapSizeX)][RandomColumn:NextInteger(1, MapSizeY)]
